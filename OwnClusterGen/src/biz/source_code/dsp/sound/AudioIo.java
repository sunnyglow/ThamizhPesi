...
public static AudioInputStream getAudioStream (AudioSignal signal, int pos, int len) throws Exception 
{
	   AudioFormat format = new AudioFormat(signal.samplingRate - 500, 16, signal.getChannels(), true, false);
	   AudioBytesPackerStream audioBytesPackerStream = new AudioBytesPackerStream(format, signal.data, pos, len);
	   AudioInputStream audioInputStream = new AudioInputStream(audioBytesPackerStream, format, len);
